...
def parse_args():
    p = argparse.ArgumentParser(
        description="Combined evaluation for SF2M (fate accuracy + W2)"
    )
    ...
    p.add_argument("--sim_modes", nargs="+", default=["ode", "sde"],
                   choices=["ode", "sde"],
                   help="Simulation modes (default: both)")
    p.add_argument("--n_sims_fate", type=int, default=100,
                   help="Trajectories per cell for fate accuracy (SDE)")
    p.add_argument("--n_sims_w2", type=int, default=10,
                   help="Trajectories per cell for W2 (SDE)")
...
def main():
    ...
    # W2: src=test tp_mid (norm_tps[1]), tgt=test tp_last (norm_tps[-1])
    ...
    for mode in args.sim_modes:
        ...
        # ── Fate accuracy ──
        if mode == "ode":
            sim_fn_fate = fate_eval.make_sf2m_ode_sim_fn(t_start=0.0, t_end=t_end_fate)
            model_arg = drift_model
            n_sims_fate = 1
        else:
            sim_fn_fate = fate_eval.make_sf2m_sde_sim_fn(
                t_start=0.0, t_end=t_end_fate,
                sigma=sigma, n_steps=args.sde_steps)
            model_arg = (drift_model, score_model)
        ...
        if mode == "ode":
            sim_fn_w2 = fate_eval.make_sf2m_ode_sim_fn(t_start=t_start_w2, t_end=t_end_w2)
            model_w2 = drift_model
            n_sims_w2 = 1
        else:
            sim_fn_w2 = fate_eval.make_sf2m_sde_sim_fn(
                t_start=t_start_w2, t_end=t_end_w2,
                sigma=sigma, n_steps=args.sde_steps)
            model_w2 = (drift_model, score_model)
            n_sims_w2 = args.n_sims_w2
