"""
SF2M Training Script
========================
Train a Schrödinger Flow to Flow Matching model (drift + score) on the Klein dataset.
...
python scripts/sf2m/02_train.py \
    ...
    --save_dir logs/sf2m/pca30/model --niters 20000 --gpu 0

python scripts/sf2m/02_train.py \
    --data_path data/klein/klein_addpop.h5ad \
    --obsm_key DM_EigenVectors --n_dims 10 --width 128 \
    --save_dir logs/sf2m/dm10/model --niters 10000 --gpu 0
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train SF2M on Klein dataset")
    ...
    p.add_argument("--sigma", type=float, default=0.05, help="SF2M sigma parameter")
...
def get_batch(FM, X, batch_size, n_times, device):
    """Build training batch from all consecutive timepoint pairs, with noise for score loss."""
    ts, xts, uts, noises = [], [], [], []
...
def main():
    ...
    drift_model = MLP(dim=args.n_dims, time_varying=True, w=args.width).to(device)
    score_model = MLP(dim=args.n_dims, time_varying=True, w=args.width).to(device)
    optimizer = torch.optim.AdamW(
        list(drift_model.parameters()) + list(score_model.parameters()),
        lr=args.lr,
    )

    log.info(f"Drift model: MLP(dim={args.n_dims}, w={args.width})")
    log.info(f"Score model: MLP(dim={args.n_dims}, w={args.width})")
    log.info(f"FM: SchrodingerBridgeConditionalFlowMatcher(sigma={args.sigma})")
    ...
    def compute_val_loss():
        ...
        for _ in range(args.n_val_batches):
            t, xt, ut, eps = get_batch(SF2M, X_val, args.batch_size, n_times, device)
            ...
            losses.append((flow_loss + score_loss).item())
        drift_model.train()
        score_model.train()
    ...
    drift_model.train()
    score_model.train()
    for i in tqdm(range(args.niters), desc="SF2M"):
        optimizer.zero_grad()
        t, xt, ut, eps = get_batch(SF2M, X, args.batch_size, n_times, device)
        lambda_t = SF2M.compute_lambda(t % 1)
        vt = drift_model(torch.cat([xt, t[:, None]], dim=-1))
        st = score_model(torch.cat([xt, t[:, None]], dim=-1))
        flow_loss = torch.mean((vt - ut) ** 2)
        ...
        if (i + 1) % args.log_freq == 0:
            log.info(f"  iter {i+1}/{args.niters}  loss={loss_val:.4f}  "
                     f"flow={flow_loss.item():.4f}  score={score_loss.item():.4f}")
        ...
        if (i + 1) % args.val_freq == 0:
            ...
            if vl < best_val_loss:
                ...
                best_iter = i + 1
                torch.save({
                    "drift_model_state_dict": drift_model.state_dict(),
