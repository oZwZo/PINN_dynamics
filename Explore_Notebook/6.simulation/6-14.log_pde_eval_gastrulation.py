...
if __name__ == '__main__':
    ...
    config_dir = "logs/gastrulation_multiscaled_expmellon/pde_params_tsense"
...
cellstate_key=ds_config['cellstate_key']
log_transform= model_dict[v].log_transform

if config.raw_args['mellon_predictor'] is None:
    t_pred = mellon.Predictor.from_json(f"data/{dataset_name}_mellon_timecontinuous_predictor.json") 
else:
    assert os.path.exists(config.raw_args['mellon_predictor'])
    t_pred = mellon.Predictor.from_json(config.raw_args['mellon_predictor']) 

def wrap_mellon_pred(x, time):
    log_u = t_pred(x,time=time)
    return np.exp(log_u)
...
for i, t in enumerate(adata.uns['pop']['t']):
    if log_transform:
        ...
        den_fun = partial(wrap_mellon_pred, time = t) 
        # den_fun = lambda x : np.exp(pred_xt(x))
...
DS_fun = lambda ad : reader.TwoTimpepoint_AnnDS(
                            AnnData=ad, 
                            timepoint_idx= ds_config['timepoint_idx'], 
                            n_dimension = ds_config['n_dimension'],
                            cellstate_key=ds_config['cellstate_key'],
                            log_transform= model_dict[v].log_transform, # decided by the model
                            split=None,
                            norm_time=ds_config['norm_time'],
                            deltax_key=ds_config['deltax_key'],
                            density_funs = predictors,
                            batchsize=500)
full_DS = DS_fun(adata)

# %%
# test predictors
X = adata.obsm[cellstate_key]
u_mellon = np.array(
    [pred_fn(X) for pred_fn in predictors]
)
fig, axs = PINN.pl.params_in_umap(adata, u_mellon, param=r'$u_b$', cell_of_t=False)
...
for v in model_dict:
    ...
    fig_ub,axs = PINN.pl.params_in_umap(adata, u_b, param=r'$u_b$', cell_of_t=False)
    # fig_ub.savefig(os.path.join(result_dir,'u_b.png'), dpi=150, transparent=True)

    fig_uint,axs = PINN.pl.params_in_umap(adata, u_int_all, param=r'$u_{sim}$', cell_of_t=False)
    # fig_uint.savefig(os.path.join(result_dir,'u_int.png'), dpi=150, transparent=True)

    fig_loguint,axs = PINN.pl.params_in_umap(adata, np.log(u_int_all+1e-10), param=r'$\log {u_{sim}}$', cell_of_t=False)
