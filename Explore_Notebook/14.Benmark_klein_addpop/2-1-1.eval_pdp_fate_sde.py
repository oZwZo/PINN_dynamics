...
fate_csv = output_path.replace(".csv", f"t{args.t_end_norm}_n{args.noise_scale}_celltype_fractions.csv")
fate_tbl.to_csv(fate_csv)
print(f"Cell-type fractions → {fate_csv}")
print(f"\n{fate_tbl.to_string()}")

# Per-cell F_hat
fhat_csv = output_path.replace(".csv", f"t{args.t_end_norm}_n{args.noise_scale}_F_hat.csv")
