...
def umap_by_time(color_col, anndata, timepoints=timepoints):
    ...
    return fig, axs
def plot_along_pseudotime(color_col, anndata, timepoints=timepoints):
    ...
    for t in timepoints:
        ...
        sns.kdeplot(anndata.obs.query('`timepoint_tx_days` == @t')['dpt_pseudotime_scaled'], label="d%d"%t, ax=axs[1,axis_j])
...
def scatter_density(color_col, anndata, timepoints=timepoints):
    fig,axs = plt.subplots(1, 9, figsize=(30,2), dpi=100, gridspec_kw={'wspace':0.3})
    # axs = axs.flatten()
    axis_j = 0
    ...
    for t in timepoints:
        cbs = anndata.obs.query('`timepoint_tx_days` == @t').index
        sns.scatterplot(data=anndata.obs.loc[cbs], x='dpt_pseudotime_scaled', y = color_col, ax=axs[axis_j])
        axis_j += 1
    return fig